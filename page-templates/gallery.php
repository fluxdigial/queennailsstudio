  ...
  <div class="container">
    ...
    <div class="gallery__tabs flex flex-wrap justify-center gap-4 mb-8">
      <?php foreach ($gallery_tabs as $index => $tab): ?>
        <button
          class="gallery__tab px-4 py-2 rounded-full border text-lg font-medium transition-all duration-300 <?= $index === 0 ? 'active bg-[#8b5e3c] text-white' : 'bg-transparent text-black' ?>"
          data-tab="tab-<?= $index; ?>">
    ...
    <?php foreach ($gallery_tabs as $index => $tab):
        ...
        <div
            id="tab-<?= $index; ?>"
            class="gallery__content <?= $index === 0 ? 'block' : 'hidden'; ?>">
            ...
            ?>
                ...
                class="gallery__item <?= $i >= 12 ? 'lazy-load' : 'active' ?>"
                data-lg-size="1400-933"
                data-sub-html="<h4><?= esc_attr($img_title); ?></h4>">
                <img
                    src="<?= $i < 12 ? esc_url($image_url) : '' ?>"
                    data-src="<?= esc_url($image_url); ?>"
                    class="gallery__image <?= $i >= 12 ? 'lazy' : 'loaded' ?>"
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      tabs.forEach(t => t.classList.remove('active', 'bg-[#8b5e3c]', 'text-white'));
      tabs.forEach(t => t.classList.add('bg-transparent', 'text-black'));

      // Thêm active cho nút được click
      tab.classList.add('active', 'bg-[#8b5e3c]', 'text-white');
