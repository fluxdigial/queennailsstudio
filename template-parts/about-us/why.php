...
<section class="why py-12 md:py-14 xl:py-16 overflow-hidden">
	<div class="container">
        <div class="grid gap-12 lg:gap-0 grid-cols-1 lg:grid-cols-2">
            <div class="why__content flex flex-col justify-between">
                ...
                <div class="why__wrapper mt-8 xl:pr-[100px] 2xl:pr-[94px]">
                    ...
                    foreach ( $whys as $index => $items ) :
                        ...
                        $delay = $index * 0.3;
                        ?>
                        <div class="why__item pb-4 mb-4 lg:mb-5 last:border-0 last:mb-0 last:pb-0 border-solid border-[1px] border-b-1 border-x-0 border-t-0 border-[#DBDBDB] wow animate__animated animate__fadeInUp" data-wow-delay="<?= esc_attr( $delay . 's' ); ?>">
                            <?php if ( ! empty( $title ) ) : ?>
                                <h3 class="why__item--title cursor-pointer text-[#000000] hover:text-heading my-0 text-[18px] xl:text-[20px] font-medium">
                                    <?= nl2br( $title ); ?>
                                </h3>
                            <?php endif; ?>
                            <?php if ( ! empty( $desc ) ) : ?>
            ...
            <div class="why__image order-first lg:order-last">
                <?php if ( ! empty( $why_image) ) : ?>
                    <img src="<?= esc_url( $why_image ) ?>" alt="<?= nl2br( esc_html( $why_title ) ); ?>"
                        loading="lazy" class="w-full aspect-square object-cover block" width="910" height="910">
                <?php endif; ?>
            </div>
        </div>
    </div>
