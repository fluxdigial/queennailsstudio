            ...
            <div class="blaze-track">
                <?php foreach ( $banners as $index => $banner ) :
                    ...
                    <div class="banners__item relative">
                        ...
                        <div class="banners__content w-full lg:w-[783px] p-4 lg:px-10 lg:py-20  text-center lg:text-left absolute">
                            <?php if ( ! empty( $banner['title'] ) ) : ?>
                                <h2 class="banners__content--heading text-center lg:text-left my-0 text-[#fff] text-[40px] md:text-[44px] lg:text-[48px] xl:text-[56px]  font-normal wow animate__animated animate__fadeInUp">
                                    <?= $banner['title'] ; ?>
                                </h2>
                            <?php endif; ?>
                            <?php if ( ! empty( $banner['content'] ) ) : ?>
                                <div class="banners__content--desc mt-5 lg:mt-6 text-center lg:text-left my-0 text-[#fff] text-[16px] lg:text-[18px]  font-light wow animate__animated animate__fadeInUp" data-wow-delay="0.3s">
                                    <?= $banner['content'] ; ?>
                                </div>
                            <?php endif; ?>
                             <?php if ( ! empty( $buttons ) ) : ?>
                                <div class="buttons__wrap mt-10 flex flex-col lg:flex-row gap-4 lg:justify-start justify-center wow animate__animated animate__fadeInUp" aria-label="Slider Button <?= $index + 1 ?>" >
                                    <?php foreach ( $buttons as $index => $items ) :
                                        ...
                                        $is_even = $index % 2 === 0;
                                        $btn_class = $is_even
                                            ? 'text-white bg-[#BA0510] hover:text-[#BA0510] hover:bg-white'
                                            : 'text-[#BA0510] bg-white hover:text-white hover:bg-[#BA0510]';
                                        ?>
                                        <?php if ( $btn_link ) : ?>
                                            <a href="<?= $btn_link ?>" class="inline-block px-6 py-3   text-[18px] font-medium hover:no-underline <?= $btn_class ?>" title="Slider Button <?= $index + 1 ?>" data-wow-delay="<?= 0.3 + ( $index + 1 ) * 0.3 ?>s">
                                                <?= esc_html( $btn_text ); ?>
                                            </a>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
