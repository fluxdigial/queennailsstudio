			...
			<div class="blaze-track">
				<?php foreach( $banners as $index => $banner ):
					...
					<div class="banners__item relative">
						<img src="<?= esc_url( $desktop_url ) ?>" alt="<?= $alt_attribute ?>" loading="<?= $index === 0 ? 'eager' : 'lazy' ?>" class="banners__image w-full hidden md:block" width="1920" height="630">

						<img src="<?= esc_url( $mobile_url ) ?>" alt="<?= $alt_attribute ?>" loading="<?= $index === 0 ? 'eager' : 'lazy' ?>" class="banners__image_mb w-full block md:hidden" width="768" height="1365">
							...
							<?php if( !empty( $buttons ) ): ?>
								<div class="buttons__wrap mt-10 flex flex-col lg:flex-row gap-4 justify-center wow animate__animated animate__fadeInUp" aria-label="Slider Button <?= $index + 1 ?>">
									<?php foreach( $buttons as $index => $items ):
										...
										?>
										<?php if( $btn_link ): ?>
											<a href="<?= $btn_link ?>" class="inline-block rounded-[30px] px-6 py-3 xl:min-w-[160px] text-[18px] font-medium hover:no-underline <?= $btn_class ?>" title="Slider Button <?= $index + 1 ?>" data-wow-delay="<?= 0.3 + ( $index + 1 ) * 0.3 ?>s">
												<?= esc_html( $btn_text ); ?>
											</a>
										<?php endif; ?>
									<?php endforeach; ?>
